  ...
  return (
    ...
    >
      ...
      <div class="md:hidden m-4">
        ...
        <div
          class={`absolute top-0 right-0 h-screen w-screen bg-white p-4 ${
            menuVisible ? "" : "hidden"
          }`}
        >
      ...
      <div class="hidden md:flex flex-col justify-start gap-3">
        ...
        {indexMenu.map(({ label, href }, index) => (
          <a
            class={`hover:text-green-500 text-sm ${
              title && title.toLowerCase() === label.toLowerCase()
                ? "text-green-500"
                : ""
            }`}
      ...
      <div class="flex flex-col items-center justify-center gap-8 px-4 py-6 md:py-8 md:px-10 w-full md:max-w-[1040px]">
        <h1 class="title-h1 uppercase flex justify-start font-semibold text-3xl w-full">
          {title}
        </h1>
        <Image
          src={content?.image || ""}
